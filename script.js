...
// gernerate HTML elements based on the trelloData

function renderColumns(){
    let trelloDataRowRootNode = document.getElementById("dataDisplayRow");
    // Removing and stale or old HTML conten:
    ...
    trelloData.columns.forEach((column) => {
        ...
        column.cards.forEach((card) => {
            // Find the card preview, copy it, save the copy to the varibale
            ...
            // Same as above but for the paragragph tag
...
function dropCard(event){
    ...
    trelloData.columns.forEach(column => {
        ...
        column.card = column.cards.filter(card => card.timestamp != oldCardData.timestamp);
